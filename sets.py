...
for b in B:
    for e in E:
        required = component_usage[b].get(e, 0)
        # sum of all y[e,b,*] == required × (1 if part b is produced somewhere)
        m.addConstr(
            gp.quicksum(y[e, b, a] for a in A)
            == required * gp.quicksum(country_part[b, a] for a in A),
            name=f"comp_usage_{b}_{e}"
        )
